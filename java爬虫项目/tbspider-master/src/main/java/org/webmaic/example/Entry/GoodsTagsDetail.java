...
public class GoodsTagsDetail implements PageProcessor {
    ...
    public void process(Page page) {
        ...
        ArrayList<ExtractGoodsArgs> list = new ArrayList<ExtractGoodsArgs>(40);

        int sentry=1;
        Date date = new Date();
        while (true){
            //如果没有下一条li就调出
            if ((page.getHtml().xpath("//*[@id=\"J_AttrUL\"]/li["+sentry+"]/text()")+"").equals("null")){
                break;
            }
            //取得当前li的内容
            String temp = page.getHtml().xpath("//*[@id=\"J_AttrUL\"]/li["+sentry+"]/text()").toString();
            //统一分隔符
            temp = temp.replace("：",":");

            ExtractGoodsArgs ega = new ExtractGoodsArgs();
            //时间
            ega.setCollectTime(date);
            //标题
            ega.setArgsName(MatcherUtil.generalMatcher(temp,"(.*):",1).trim());
            //内容
            ega.setArgsValues(MatcherUtil.generalMatcher(temp,":(.*)",1).trim());
            //goodsId
            ega.setGoodsId(Long.parseLong(MatcherUtil.generalMatcher(page.getHtml().toString(),"item/(.*?[0-9])\\.htm",1)));
           //来源
            ega.setDataSource(DataSource.TMall.getDesc());
    ...
    public static void main(String[] args) {
        ...
        Spider.create(new GoodsTagsDetail())
