...
import org.webmaic.example.model.ExtractAssessTags;
 ...
 * 评论标签
 */
public class AssessTags implements PageProcessor {
    ...
    public void process(Page page) {
        ...
        List<ExtractAssessTags> tagList = new ArrayList<ExtractAssessTags>(20);
        Date date = new Date();

        //标签名称
        List<String> tagNameList = new JsonPathSelector("$.tags.tagClouds[*].tag").selectList(handleRawText);
        //标签数量
        List<String> tagNumList = new JsonPathSelector("$.tags.tagClouds[*].count").selectList(handleRawText);
        //标签颜色
        List<String> tagPosiList = new JsonPathSelector("$.tags.tagClouds[*].posi").selectList(handleRawText);
        //标签Id
        List<String> idList = new JsonPathSelector("$.tags.tagClouds[*].id").selectList(handleRawText);
        //商品Id
        String goodsId = MatcherUtil.generalMatcher(page.getUrl().toString(),"itemId=([0-9]+)",1);

        for (int i =0;i<tagNameList.size();i++){
            ExtractAssessTags at = new ExtractAssessTags();
            at.setCollectTime(date);
            at.setDataSource(DataSource.TMall.getDesc());
            at.setGoodsId(Long.parseLong(goodsId));
            at.setTagContent(tagNameList.get(i));
            at.setTagNum(Integer.parseInt(tagNumList.get(i)));
            at.setTagId(Long.parseLong(idList.get(i)));
            if (tagPosiList.get(i).equals("true")){
                at.setPosi(1);
            }else{
                at.setPosi(0);
            }
    ...
    public static void main(String[] args) {
        ...
        Spider.create(new AssessTags())
                .addUrl("https://rate.tmall.com/listTagClouds.htm?itemId=562133187987&isAll=true&isInner=true&t=1538206232373&groupId=&_ksTS=1538206232374_786&callback=jsonp787")
                .run();
    }
