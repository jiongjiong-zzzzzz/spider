...
def standard_work_list():
    ...
    res = requests.get('http://gdj.beijing.gov.cn/zwxx/gzdt/', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('UTF-8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.select('.txt a')
    for one_info in infos:
        ...
        if content_dir:
            _datetime = 0
            if '../../' in _one_info:
                _url_tmp = content_dir['href'].replace('../../', 'http://gdj.beijing.gov.cn/')
            else:
                _url_tmp = content_dir['href'].replace('./','http://gdj.beijing.gov.cn/zwxx/gzdt/')
...
def standard_work_article(target_url):
    ...
    _title = html_page.select('div.detail div:nth-of-type(1)')[0].text.replace('\n','').replace('\r','').replace('\t','')
