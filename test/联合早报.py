...
def standard_work_list():
    ...
    res = requests.get('http://www.zaobao.com/sea/politic', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('UTF-8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='post-list view-content').findAll('a')
    for one_info in infos:
        _one_info = str(one_info)
        content_dir = one_info
        if 'post-title'not in _one_info:
            continue
        if content_dir:
            _datetime = 0
            _url_tmp = 'http://www.zaobao.com'+content_dir['href'].strip()
...
def standard_work_article(target_url):
    ...
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(id='FineDining').findAll('p')
    for one_info in infos:
    ...
    tim = html_page.find(class_='datestamp date-published meta-date-published').text.strip()
