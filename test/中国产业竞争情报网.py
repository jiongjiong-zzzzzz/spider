...
def standard_work_list():
    ...
    res = requests.get('http://www.chinacir.com.cn/hyzx/01/', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('gb2312')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='news_list3').findAll('a')
    for one_info in infos:
        ...
        if content_dir:
            ...
            _url_tmp = _url_tmp.replace('../../','http://www.chinacir.com.cn/')
            content_dir = content_dir.text.replace('\n','').replace('\t','')
            print({'url': _url_tmp, 'title': content_dir})
            return_data.append({'url': _url_tmp, 'title': content_dir, 'datetime': _datetime})
    return return_data
...
def standard_work_article(target_url):
    ...
    infos = html_page.find(class_='dir').findAll('p')
    ...
    if not return_data:
        return_data.append(html_page.find(class_='dir').text)
    tim = re.search(r"(\d{4}-\d{1,2}-\d{1,2})", reg_content)
    ...
    _title = html_page.find(class_='dirTitle').text
