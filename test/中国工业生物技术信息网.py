...
def standard_work_list():
    ...
    res = requests.get('http://www.bioindustry.cn/main/topic/nengyuan?page=1', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf-8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(id='news').findAll('a')
    for one_info in infos:
        ...
        if content_dir:
            _datetime = 0
            _url_tmp ='http://www.bioindustry.cn'+ content_dir['href'].strip()
...
def standard_work_article(target_url):
    ...
    reg_content = res.content.decode('utf-8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    need_content = html_page.find(id='nr').text
