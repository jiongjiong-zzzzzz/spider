...
def standard_work_list():
    ...
    res = requests.get('https://www.chinalaw.org.cn/', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.select('div.toplist ul li a')
    for one_info in infos:
        ...
        if content_dir:
            ...
            if 'InfoID' not in _url_tmp:
                continue
            _url_tmp = 'https://www.chinalaw.org.cn'+_url_tmp
...
def standard_work_article(target_url):
    ...
    reg_content = res.content.decode('utf8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='xx_con').findAll('p')
