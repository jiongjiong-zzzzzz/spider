...
def standard_work_list():
    ...
    res = requests.get('http://www.mee.gov.cn/xxgk/hjyw/', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('UTF-8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='main_rt_list').findAll('a')
    for one_info in infos:
        ...
        if content_dir:
            ...
            _url_tmp = _url_tmp.replace('../../','http://www.mee.gov.cn/')
            _url_tmp = _url_tmp.replace('./', 'http://www.mee.gov.cn/xxgk/hjyw/')
...
def standard_work_article(target_url):
    ...
    if not return_data:
        return_data.append(html_page.find(class_='content_body_box').text.strip())
