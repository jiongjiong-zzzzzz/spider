...
def standard_work_list():
    ...
    res = requests.get('http://www.safe.gov.cn/safe/sjjd/index.html', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('UTF-8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='list_conr').findAll('a')
    for one_info in infos:
        ...
        if content_dir:
            _datetime = 0
            _url_tmp = 'http://www.safe.gov.cn'+content_dir['href']
