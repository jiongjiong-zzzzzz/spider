...
def standard_work_list():
    ...
    res = requests.get('http://www.mohurd.gov.cn/czjs/index.html', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf-8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = re.findall('<a href=".+?">.+?</a>', reg_content)
    for one_info in infos:
        ...
        if content_dir:
            ...
            if 'index' in _url_tmp or 'jbxxk' in _url_tmp or 'cblack' in _url_tmp or 'img'in _url_tmp:
                continue
...
def standard_work_article(target_url):
    ...
    reg_content = res.content.decode('utf-8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='union').findAll('p')
    for one_info in infos:
