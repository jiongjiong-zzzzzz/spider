...
def standard_work_list():
    ...
    res = requests.get('http://www.zyczyc.com/info/WenZhang.aspx?lmid=11', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('UTF-8','ignore')
    infos = re.findall('<a class="lvse11ptcu" href="(?P<url>.+?)" target="_blank">(?P<title>.+?)</a>', reg_content)
    ...
    for one_info in infos:
        ...
        if content_dir:
            _datetime = 0
            _url_tmp = 'http://www.zyczyc.com/info/'+content_dir[0]
...
def standard_work_article(target_url):
    ...
    infos = html_page.findAll(class_='bjh-p')
    for info in infos:
        return_data.append(info.text)
    if not return_data:
        return_data.append(html_page.find(class_='webnr').text)
    ...
    _title = html_page.find('title').text.replace('\r\n','')
