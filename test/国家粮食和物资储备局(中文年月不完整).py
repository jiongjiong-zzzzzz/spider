...
def standard_work_list():
    ...
    res = requests.get('http://www.lswz.gov.cn/html/xinwen/index.shtml', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf-8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='lists diylist').findAll('a')
    for one_info in infos:
        ...
        if content_dir:
            ...
            _url_tmp='http://www.lswz.gov.cn'+_url_tmp
...
def standard_work_article(target_url):
    ...
    reg_content = res.content.decode('utf-8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='pub-det-content').findAll('p')
    for one_info in infos:
