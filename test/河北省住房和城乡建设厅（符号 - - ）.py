...
def standard_work_list():
    ...
    res = requests.get('http://zfcxjst.hebei.gov.cn/xinwenzhongxin/gongzuodongtai/', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf-8','ignore')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.select('.rgtbar_erjilist li a')
    for one_info in infos:
        ...
        if content_dir:
            _datetime = 0
            _url_tmp = content_dir['href'].replace('./','http://zfcxjst.hebei.gov.cn/xinwenzhongxin/gongzuodongtai/')
...
def standard_work_article(target_url):
    ...
    reg_content = res.content.decode('utf-8','ignore')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='conbox').text
    ...
    _title = html_page.select('.conbox h1')[0].text
