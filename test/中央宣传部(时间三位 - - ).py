...
def standard_work_list():
    ...
    res = requests.get('http://www.wenming.cn/a/yw/', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.select('ul.fbt-list li div.fbt-p a')
    for one_info in infos:
        ...
        if content_dir:
            ...
            if 'wenming' not in _url_tmp:
                _url_tmp = content_dir['href'].replace('./','http://www.wenming.cn/ziliao/wenjian/jigou/zhongxuanbu/')
