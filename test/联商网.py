...
def standard_work_list():
    ...
    res = requests.get('http://www.linkshop.com.cn/web/artlist_point.html', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('gb2312')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='readlist_cont').findAll('a')
...
def standard_work_article(target_url):
    ...
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='page').findAll('p')
    for one_info in infos:
    ...
    datetime_dir = re.match('(?P<year>\d{4})-(?P<month>\d+?)-(?P<day>\d+?) (?P<hour>\d+?):(?P<minute>\d+?)',
                            html_page.find(class_='time').text.strip())
    tt_tmp = '%s-%s-%s %s:%s' % (
