...
def standard_work_list():
    ...
    res = requests.get('http://m.ftchinese.com/channel/world.html', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('UTF-8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.findAll('a',{'class','item-headline-link'})
    for one_info in infos:
        _one_info = str(one_info)
        if 'premium' in _one_info:
            continue
        content_dir = one_info
        if content_dir:
            _datetime = 0
            _url_tmp = 'http://m.ftchinese.com'+content_dir['href'].strip()
...
def standard_work_article(target_url):
    ...
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(id='story-body-container').findAll('p')
    for one_info in infos:
    ...
    tim = html_page.find(class_='story-time').text.strip()
