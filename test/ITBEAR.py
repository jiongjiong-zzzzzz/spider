...
def standard_work_list():
    ...
    res = requests.get('http://www.itbear.com.cn/c29.aspx', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('UTF-8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.findAll('a',{'class':'nLink'})
    for one_info in infos:
        content_dir = one_info
        if content_dir:
            _datetime = 0
            _url_tmp = 'http://www.itbear.com.cn/' + content_dir['href'].strip()
