...
def standard_work_list():
    ...
    res = requests.get('http://gzw.beijing.gov.cn/xxfb/gzyw/', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('UTF-8',"ignore")
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.select('.public_list_team a')
    for one_info in infos:
        _one_info = str(one_info)
        content_dir = one_info
        if content_dir:
            _datetime = 0
            _url_tmp = content_dir['href'].strip().replace('./','http://gzw.beijing.gov.cn/xxfb/gzyw/')
...
def standard_work_article(target_url):
    ...
    reg_content = res.content.decode('utf-8','ignore')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(id='div_zhengwen').findAll('p')
    for one_info in infos:
    ...
    if not return_data:
        return_data.append(html_page.find(id='div_zhengwen').text)
    ...
    date = html_page.find(class_='article-info').text.replace('\n','').replace(' ','')
    date = re.search(r"(\d{4}%s\d{1,2}%s\d{1,2}%s\d{1,2}:\d{1,2})"%(nian,yue,ri) , date)
    ...
    _title = html_page.select('.details_page p')[0].text.replace('\n','').replace(' ','')
