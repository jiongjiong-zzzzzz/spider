...
def standard_work_list():
    ...
    res = requests.get('http://www.100ppi.com/forecast/list-15-11-1.html', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='list-c').findAll('a')
    for one_info in infos:
        _one_info = str(one_info)
        content_dir = re.match('<a.+href="(?P<url>.+?)" target="_blank">(?P<title>.+?)</a>', _one_info)
        if content_dir:
            _datetime = 0
            _url_tmp = 'http://www.100ppi.com/forecast/'+content_dir['url'].strip()
...
def standard_work_article(target_url):
    ...
    reg_content = res.content.decode('utf8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='nd-c').findAll('p')
    for one_info in infos:
    ...
    tim = html_page.find(class_='nd-info').text.strip()
