...
def standard_work_list():
    ...
    res = requests.get('https://www.fmprc.gov.cn/web/wjdt_674879/fyrbt_674889/', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('UTF-8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='rebox_news').findAll('a')
    for one_info in infos:
        ...
        if content_dir:
            _datetime = 0
            _url_tmp = content_dir['href'].replace('./','https://www.fmprc.gov.cn/web/wjdt_674879/fyrbt_674889/').replace('amp;','')
