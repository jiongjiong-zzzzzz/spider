...
def standard_work_list():
    ...
    res = requests.get('http://kjt.jl.gov.cn/xwzx/zwdt/', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf-8','ignore')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.select('td.td14biaot a')
    for one_info in infos:
        ...
        if content_dir:
            ...
            _url_tmp = _url_tmp.replace('./','http://kjt.jl.gov.cn/xwzx/zwdt/')
...
def standard_work_article(target_url):
    ...
    _title = html_page.find(class_='font30').text
