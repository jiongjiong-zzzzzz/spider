...
def standard_work_list():
    ...
    res = requests.get('http://fgw.beijing.gov.cn/index/xwfb/', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf-8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.select('ul.lbox2 li a')
    for one_info in infos:
        ...
        if content_dir:
            ...
            if 'zwxx' in _url_tmp:
                _url_tmp=_url_tmp.replace('../../','http://fgw.beijing.gov.cn/')
            else:
                _url_tmp = _url_tmp.replace('./', 'http://fgw.beijing.gov.cn/index/xwfb/')
