...
def standard_work_list():
    ...
    res = requests.get('http://www.china-nengyuan.com/news/news_list.php?keyword=%E7%94%9F%E7%89%A9%E8%B4%A8%E7%87%83%E6%96%99&keyword2=%E7%94%9F%E7%89%A9%E7%87%83%E6%96%99&k', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf-8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.findAll('a',{'class':'f14 blue'})
    for one_info in infos:
        _datetime = 0
        content_dir = one_info
        _url_tmp = 'http://www.china-nengyuan.com'+content_dir['href'].strip()
...
def standard_work_article(target_url):
    ...
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='f14 news_link').findAll('p')
    for one_info in infos:
