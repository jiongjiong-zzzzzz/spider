...
def standard_work_list():
    ...
    res = requests.get('https://mil.ifeng.com/shanklist/14-35083-', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.findAll('h2')
    for one_info in infos:
        _one_info = str(one_info)
        content_dir = re.match('<h2 class="news-stream-newsStream-news-item-has-image clearfix news_item"><a href="(?P<url>.+?)".+>(?P<title>.+?)</a></h2>', _one_info)
        if content_dir:
            _datetime = 0
...
def standard_work_article(target_url):
    ...
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='text-3zQ3cZD4').findAll('p')
    for one_info in infos:
