...
def standard_work_list():
    ...
    res = requests.get('http://www.ccps.gov.cn/xwpd/rdxw/', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('UTF-8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='wListUl').findAll('a')
    for one_info in infos:
        _one_info = str(one_info)
        content_dir = one_info
        if content_dir:
            _datetime = 0
            _url_tmp = content_dir['href'].replace('./','http://www.ccps.gov.cn/xwpd/rdxw/')
...
def standard_work_article(target_url):
    ...
    date = html_page.find(class_='xlTimeBox').text
