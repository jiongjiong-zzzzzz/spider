...
def standard_work_list():
    ...
    res = requests.get('http://yjj.guizhou.gov.cn/xwdt/jgdt/', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf-8','ignore')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.select('.bd li a')
    for one_info in infos:
        ...
        if content_dir:
            ...
            if 'http' not in _one_info:
                _url_tmp =content_dir['href']
            print({'url': _url_tmp, 'title': content_dir['href'].strip()})
            return_data.append({'url': _url_tmp, 'title': content_dir['href'].strip(), 'datetime': _datetime})
    return return_data
...
def standard_work_article(target_url):
    ...
    _title = html_page.find(class_='Article_bt').text.replace('\n','').replace('\t','').replace('\r','')
