...
def standard_work_list():
    ...
    res = requests.get('http://hbepb.hebei.gov.cn/xwzx/stdt/', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('gbk','ignore')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.select('.erjilist li a')
    for one_info in infos:
        ...
        if content_dir:
            _datetime = 0
            _url_tmp = content_dir['href'].replace('./','http://hbepb.hebei.gov.cn/xwzx/stdt/')
