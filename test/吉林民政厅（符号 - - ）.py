...
def standard_work_list():
    ...
    res = requests.get('http://mzt.jl.gov.cn/mzyw_74261/', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf-8','ignore')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.select('.zly_mzt_20170310mzywul a')
    for one_info in infos:
        ...
        if content_dir:
            ...
            if '../' in _url_tmp:
                _url_tmp = _url_tmp.replace('../', 'http://mzt.jl.gov.cn/')
            _url_tmp = _url_tmp.replace('./','http://mzt.jl.gov.cn/mzyw_74261/')
...
def standard_work_article(target_url):
    ...
    if datetime_dir:
        ...
        _title = html_page.find(class_='zly_xq_20170316_t').text
