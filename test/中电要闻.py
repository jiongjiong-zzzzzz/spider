...
def standard_work_list():
    ...
    res = requests.get('http://www.cpnn.com.cn/zdyw/default.htm', headers=headers)
    ...
    for one_info in infos:
        ...
        if 'target'in _one_info:
            continue
        content_dir = re.match('<a href="(?P<url>.+?)">(?P<title>.+?)</a>', _one_info)
        if content_dir:
            _datetime = 0
            _url_tmp = content_dir['url'].replace('./','http://www.cpnn.com.cn/zdyw/').replace('amp;','')
...
def standard_work_article(target_url):
    ...
    try:
        ...
        infos = html_page.find(id='postmessage_95918').findAll('p')
    for one_info in infos:
