...
def standard_work_list():
    ...
    res = requests.get('http://sfj.beijing.gov.cn/bjsf/zwxx/index.html', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf-8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.select('.zwgk_left.column .zw_list padt10,.zw_list a')
    for one_info in infos:
        ...
        if content_dir:
            ...
            _url_tmp='http://sfj.beijing.gov.cn'+_url_tmp
