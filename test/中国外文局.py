...
def standard_work_list():
    ...
    res = requests.get('http://www.cipg.org.cn/xwzx/', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.select('ul li#li_height a')
    for one_info in infos:
        ...
        if content_dir:
            ...
            if 'tv.cctv' in _url_tmp:
                continue
            _url_tmp = _url_tmp.replace('./','http://www.cipg.org.cn/xwzx/')
