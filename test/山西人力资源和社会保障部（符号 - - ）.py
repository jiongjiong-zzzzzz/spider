...
def standard_work_list():
    ...
    res = requests.get('http://rst.shanxi.gov.cn/zwyw/gzdt/szyw/', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf-8','ignore')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.select('.second_right_ul li a')
    for one_info in infos:
        ...
        if content_dir:
            _datetime = 0
            _url_tmp = content_dir['href'].replace('./','http://rst.shanxi.gov.cn/zwyw/gzdt/szyw/')
