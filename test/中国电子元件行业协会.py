...
def standard_work_list():
    ...
    res = requests.get('http://www.ic-ceca.org.cn/hydt/index.jhtml', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='newsList content').findAll('a')
...
def standard_work_article(target_url):
    ...
    infos = html_page.find(class_='main-body').findAll('p')
