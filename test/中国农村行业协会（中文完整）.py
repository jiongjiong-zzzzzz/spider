...
def standard_work_list():
    ...
    res = requests.get('http://www.carei.org.cn/', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('UTF-8',"ignore")
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(id='ulList_con_99_1').findAll('a')
    for one_info in infos:
        _one_info = str(one_info)
        content_dir = one_info
        if content_dir:
            _datetime = 0
            _url_tmp = 'http://www.carei.org.cn'+content_dir['href']
...
def standard_work_article(target_url):
    ...
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    try:
        infos = html_page.find(class_='w-detail').findAll('p')
    except:
        return
    for one_info in infos:
    ...
    date = re.search(r"(\d{4}%s\d{1,2}%s\d{1,2}%s\D+\d{1,2}:\d{1,2})"%(nian,yue,ri) , reg_content)
    da = date[0]
    datetime_dir = re.match('(?P<year>\d{4})%s(?P<month>\d+?)%s(?P<day>\d+?)%s\D+(?P<hour>\d+?):(?P<minute>\d+)'%(nian,yue,ri),date[0])
    tt_tmp = '%s-%s-%s %s:%s' % (
    ...
    _title = html_page.find(class_='w-title').text.replace('\r','').replace('\n','')
