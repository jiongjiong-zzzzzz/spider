...
def standard_work_list():
    ...
    res = requests.get('http://www.qizhiwang.org.cn/GB/422352/index.html', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('GB2312',"ignore")
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.select('ul.list_14 li a')
    for one_info in infos:
        _one_info = str(one_info)
        content_dir =one_info
        if content_dir:
            _datetime = 0
            _url_tmp = 'http://www.qizhiwang.org.cn'+content_dir['href'].strip()
...
def standard_work_article(target_url):
    ...
    reg_content = res.content.decode('GB2312','ignore')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='text_con w1000 clearfix').findAll('p')
    for one_info in infos:
