...
def standard_work_list():
    ...
    res = requests.get('http://www.gjxfj.gov.cn/gjxfj/news/zyxw/ttxw/A09020101index_1.htm', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('UTF-8')
    infos = re.findall('<a class="pull-left" href="(?P<url>.+?)" target="_blank" title=".+?">(?P<title>.+?)</a>', reg_content)
    ...
    for one_info in infos:
        ...
        if content_dir:
            _datetime = 0
            _url_tmp = 'http://www.gjxfj.gov.cn'+content_dir[0]
            if 'zzjg' in _url_tmp:
                continue
...
def standard_work_article(target_url):
    ...
    infos = html_page.find(id='BodyLabel').findAll('p')
    for one_info in infos:
    ...
    if not return_data:
        return_data.append(html_page.find(id='txt').text)
