...
def standard_work_list():
    ...
    res = requests.get('http://mzw.guizhou.gov.cn/index.php?m=content&c=index&a=lists&catid=28', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf-8','ignore')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.select('.nlst.nlft.clearfix li a')
    for one_info in infos:
        ...
        if content_dir:
            ...
            if 'http' not in _one_info:
                _url_tmp = 'http://mzw.guizhou.gov.cn'+content_dir['href']
...
def standard_work_article(target_url):
    ...
    reg_content = res.content.decode('utf-8','ignore')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='nrfont').text
