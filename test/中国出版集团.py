...
def standard_work_list():
    ...
    res = requests.get('http://cn.cnpubg.com/news/jtxw/', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.select('.block.w-565.h-24.o-h.ellipsis a.black')
...
def standard_work_article(target_url):
    ...
    reg_content = res.content.decode('utf8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='w-747 mc mt-20 news_content').findAll('p')
    for one_info in infos:
