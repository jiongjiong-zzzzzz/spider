...
def standard_work_list():
    ...
    res = requests.get('http://www.cnecc.org.cn/news.asp?classid=5&nclassid=16#', headers=headers)
    ...
    for one_info in infos:
        ...
        if 'img' in _one_info:
            continue
        _one_info = _one_info.replace('\r','').replace('\n','').replace('\t','')
        content_dir = re.match('<a href="(?P<url>.+?)" target="_blank">(?P<title>.+?)</a>', _one_info)
        if content_dir:
            _datetime = 0
            _url_tmp = 'http://www.cnecc.org.cn/'+content_dir['url'].strip()
...
def standard_work_article(target_url):
    ...
    try:
        ...
        return
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='content_td_font').findAll('p')
    for one_info in infos:
