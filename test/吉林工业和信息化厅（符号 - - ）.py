...
def standard_work_list():
    ...
    res = requests.get('http://gxt.jl.gov.cn/gxdt1/gxyw/', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('gbk','ignore')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.select('.glyleft2_ly li a')
    for one_info in infos:
        ...
        if content_dir:
            ...
            _url_tmp = _url_tmp.replace('./','http://gxt.jl.gov.cn/gxdt1/gxyw/')
...
def standard_work_article(target_url):
    ...
    _title = html_page.find(class_='news_tit_ly').text
