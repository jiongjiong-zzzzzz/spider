...
def standard_work_list():
    ...
    headers = {'User-Agent': UserAgent,
               'Cookie':'maxPageNum8052=190; _gscu_1226177633=52274490timwj536; _gscbrs_1226177633=1; _gscs_1226177633=53222555igsktg15|pv:4'}
    res = requests.get('http://www.sport.gov.cn/n316/n337/index.html', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('UTF-8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = re.findall('<a href="(?P<url>.+?)" target=_blank title=".+?">(?P<title>.+?)</a>', reg_content)
    ...
    for one_info in infos:
        ...
        if content_dir:
            _datetime = 0
            _url_tmp = content_dir[0].replace('../../','http://www.sport.gov.cn/')
...
def standard_work_article(target_url):
    ...
    reg_content = res.content.decode('UTF-8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='wz_con').findAll('p')
    for one_info in infos:
    ...
    _title = html_page.find(class_='wz_title').text.replace('\r\n','')
