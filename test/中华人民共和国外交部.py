...
def standard_work_list():
    ...
    res = requests.get('https://www.fmprc.gov.cn/web/wjbxw_673019/', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='imbox').findAll('a')
    for one_info in infos:
        ...
        if content_dir:
            ...
            if 'www' not in _url_tmp:
                _url_tmp = _url_tmp.replace('./','https://www.fmprc.gov.cn/web/wjbxw_673019/')
...
def standard_work_article(target_url):
    ...
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(id='News_Body_Txt_A').findAll('p')
    for one_info in infos:
