...
def standard_work_list():
    ...
    res = requests.get('http://www.mwr.gov.cn/xw/slyw/', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='slnewscon autoHeight').findAll('a')
    for one_info in infos:
        _one_info = str(one_info)
        if 'index' in _one_info or 'vod.mwr.gov' in _one_info:
            continue
        ...
        if content_dir:
            ...
            _url_tmp =_url_tmp.replace('./','http://www.mwr.gov.cn/xw/slyw/')
