...
def standard_work_list():
    ...
    res = requests.get('http://mzt.shanxi.gov.cn/newslist/74.html', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf-8','ignore')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.select('.news a')
    for one_info in infos:
        _one_info = str(one_info)
        content_dir = one_info
        if content_dir:
            _datetime = 0
            _url_tmp = 'http://mzt.shanxi.gov.cn'+content_dir['href']
...
def standard_work_article(target_url):
    ...
    reg_content = res.content.decode('utf-8','ignore')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='newscontent').findAll('p')
    if not infos:
        return
    for one_info in infos:
    ...
    _title = html_page.select('.newsxxtitle span')[0].text
