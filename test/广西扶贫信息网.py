...
def standard_work_list():
    ...
    res = requests.get('http://www.gdfp.gov.cn/fpyw/jdxw/', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='right_list_news').findAll('a')
    for one_info in infos:
        ...
        if content_dir:
            ...
            _url_tmp = _url_tmp.replace('./','http://www.gdfp.gov.cn/fpyw/jdxw/')
...
def standard_work_article(target_url):
    ...
    reg_content = res.content.decode('utf8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='content_article_info').findAll('p')
    for one_info in infos:
