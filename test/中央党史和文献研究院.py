...
def standard_work_list():
    ...
    res = requests.get('https://www.wxyjs.org.cn/zyldrhd_547/', headers=headers,verify=False)
    res.raise_for_status()
    reg_content = res.content.decode('utf8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='center gl_list1').findAll('a')
    for one_info in infos:
        _one_info = str(one_info)
        content_dir = one_info
        if content_dir:
            _datetime = 0
            _url_tmp = content_dir['href'].replace('./','https://www.wxyjs.org.cn/zyldrhd_547/')
