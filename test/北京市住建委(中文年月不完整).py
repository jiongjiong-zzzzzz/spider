...
def standard_work_list():
    ...
    res = requests.get('http://zjw.beijing.gov.cn/bjjs/xxgk/xwfb/index.shtml', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf-8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.select('.list_box ul a')
    for one_info in infos:
        _one_info = str(one_info)
        content_dir = one_info
        if content_dir:
            _datetime = 0
            _url_tmp = 'http://zjw.beijing.gov.cn'+content_dir['href'].strip()
