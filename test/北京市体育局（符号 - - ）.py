...
def standard_work_list():
    ...
    res = requests.get('http://tyj.beijing.gov.cn/bjsports/gzdt84/tytj/index.html', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('UTF-8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.select('.news_list_time li a')
    for one_info in infos:
        ...
        if content_dir:
            _datetime = 0
            _url_tmp = 'http://tyj.beijing.gov.cn'+content_dir['href']
