...
def standard_work_list():
    ...
    res = requests.get('http://kjt.shanxi.gov.cn/gzdt/index.jhtml', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf-8','ignore')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.select('#container > div.main > div:nth-of-type(3) > table > tbody > tr > td:nth-of-type(2) > a')
