...
def standard_work_list():
    ...
    res = requests.get('http://qnzz.youth.cn/tegao/', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('gb2312')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='tj3_1').findAll('a')
    for one_info in infos:
        ...
        if content_dir:
            ...
            _url_tmp = _url_tmp.replace('./','http://qnzz.youth.cn/tegao/')
            if 'v.youth' in _url_tmp or 'mp.weixin' in _url_tmp or 'picture.youth' in _url_tmp:
                continue
