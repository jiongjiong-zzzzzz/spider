...
def standard_work_list():
    ...
    res = requests.get('http://zfcxjst.guizhou.gov.cn/jszx/jqdt/', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf-8','ignore')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.select('.NewsList ul li a')
    for one_info in infos:
        ...
        if content_dir:
            ...
            if 'http' not in _one_info:
                _url_tmp = content_dir['href'].replace('./','http://czt.guizhou.gov.cn/xwzx/czdt/')
