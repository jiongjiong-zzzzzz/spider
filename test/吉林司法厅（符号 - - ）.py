...
def standard_work_list():
    ...
    res = requests.get('http://sft.jl.gov.cn/tnyw/tnyw_111933/', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf-8','ignore')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.select('.t_940_18 a')
    for one_info in infos:
        ...
        if content_dir:
            ...
            _url_tmp = _url_tmp.replace('./','http://sft.jl.gov.cn/tnyw/tnyw_111933/')
...
def standard_work_article(target_url):
    ...
    _title = html_page.find(class_='t_1200_16').text
