...
def standard_work_list():
    ...
    res = requests.get('http://news.bjx.com.cn/list?catid=100', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('gb2312')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='list_left_ul').findAll('a')
    for one_info in infos:
        _one_info = str(one_info)
        content_dir = re.match('<a href="(?P<url>.+?)" title=".+">(?P<title>.+?)</a>', _one_info)
        if content_dir:
            _datetime = 0
...
def standard_work_article(target_url):
    ...
    try:
        tim = html_page.find(class_='tempa list_copy btemp').text.strip()
    except:
        tim = html_page.find(class_='list_copy').text.strip()
    date = re.search(r"(\d{4}/\d{1,2}/\d{1,2}\s\d{1,2}:\d{1,2})" , tim)
