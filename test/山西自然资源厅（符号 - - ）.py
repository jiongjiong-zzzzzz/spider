...
def standard_work_list():
    ...
    res = requests.get('http://zrzyt.shanxi.gov.cn/xw/chnl88/', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf-8','ignore')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.select('.zwgk_right_content li a')
    for one_info in infos:
        ...
        if content_dir:
            _datetime = 0
            _url_tmp = content_dir['href'].replace('./','http://zrzyt.shanxi.gov.cn/xw/chnl88/')
