...
def standard_work_list():
    ...
    res = requests.get('http://sthjt.jl.gov.cn/', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf-8','ignore')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.select('.index_news_list a')
    for one_info in infos:
        ...
        if content_dir:
            ...
            _url_tmp = _url_tmp.replace('./','http://sthjt.jl.gov.cn/')
            if not content_dir.text.strip():
                continue
...
def standard_work_article(target_url):
    ...
    _title = html_page.find(class_='title-third').text
