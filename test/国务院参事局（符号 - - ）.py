...
def standard_work_list():
    ...
    res = requests.get('http://www.counsellor.gov.cn/sgyw/index.shtml', headers=headers)
    ...
    for one_info in infos:
        ...
        if content_dir:
            _datetime = 0
            _url_tmp = 'http://www.counsellor.gov.cn'+content_dir[0]
            if 'zzjg' in _url_tmp:
                continue
...
def standard_work_article(target_url):
    ...
    infos = html_page.find(id='txt').findAll('p')
    for one_info in infos:
    ...
    _title = html_page.find('h1').text.replace('\r\n','')
