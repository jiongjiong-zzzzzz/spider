...
def standard_work_list():
    ...
    res = requests.get('http://info.cncma.org/list/list_hangyxw.shtml', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('gb2312')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='list1').findAll('a')
    for one_info in infos:
        _one_info = str(one_info)
        if 'list' in _one_info:
            continue
        content_dir = one_info
        if content_dir:
            _datetime = 0
            _url_tmp = 'http://info.cncma.org'+content_dir['href'].strip()
...
def standard_work_article(target_url):
    ...
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    try:
        infos = html_page.find(id='artical').findAll('p')
    except:
        return
    for one_info in infos:
