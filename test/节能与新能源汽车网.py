...
def standard_work_list():
    ...
    res = requests.get('http://www.chinaev.org/DisplayView/Normal/News/QueryResult.aspx', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('UTF-8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='ui-box-content newslist').findAll('a')
    for one_info in infos:
        ...
        if content_dir:
            _datetime = 0
            _url_tmp = 'http://www.chinaev.org/DisplayView/Normal/News/'+content_dir['url'].strip()
...
def standard_work_article(target_url):
    ...
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(id='content').findAll('p')
    for one_info in infos:
    ...
    tim = html_page.find(class_='pubtime').text.strip()
