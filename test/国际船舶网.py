...
def standard_work_list():
    ...
    res = requests.get('http://www.eworldship.com/app/news/listview?id=781', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf-8','ignore')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.findAll('h2',{'class':'fB'})
    for one_info in infos:
        _one_info = str(one_info)
        content_dir = re.match('<h2 class="fB"><a href="(?P<url>.+?)">(?P<title>.+?)</a></h2>', _one_info)
        if content_dir:
            _datetime = 0
            _url_tmp = content_dir['url'].strip()
            if 'www.citnews' not in _url_tmp:
                _url_tmp = 'http://www.eworldship.com'+_url_tmp
...
def standard_work_article(target_url):
    ...
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(id='nArticle').findAll('p')
    for one_info in infos:
    ...
    return target_url, '%s<replace title>%s' % (_title, '\n'.join(return_data))
