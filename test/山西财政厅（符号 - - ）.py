...
def standard_work_list():
    ...
    res = requests.get('http://czt.shanxi.gov.cn/html/article/list/index_1147999187073_1.html', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf-8','ignore')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.select('.cont li a')
    for one_info in infos:
        ...
        if content_dir:
            _datetime = 0
            _url_tmp = 'http://czt.shanxi.gov.cn'+content_dir['href']
...
def standard_work_article(target_url):
    ...
    reg_content = res.content.decode('utf-8','ignore')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='art').findAll('p')
    if not infos:
        return
    for one_info in infos:
