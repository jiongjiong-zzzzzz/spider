...
def standard_work_list():
    ...
    res = requests.get('http://www.cnelc.com/Article/ArticleList6/', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('gb2312','ignore')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='brand_news_list').findAll('a')
    for one_info in infos:
        ...
        _url_tmp = 'http://www.cnelc.com'+content_dir['href'].strip()
...
def standard_work_article(target_url):
    ...
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='brand_news_con').findAll('p')
    for one_info in infos:
