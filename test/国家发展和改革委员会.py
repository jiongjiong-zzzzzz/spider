...
def standard_work_list():
    ...
    res = requests.get('http://www.ndrc.gov.cn/xwzx/xwfb/', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='list_02 clearfix').findAll('a')
    for one_info in infos:
        ...
        if content_dir:
            ...
            if 'www' not in _url_tmp:
                _url_tmp = _url_tmp.replace('./','http://www.ndrc.gov.cn/xwzx/xwfb/')
            if 'ft.people.com' in  _url_tmp or 'index' in _url_tmp:
                continue
...
def standard_work_article(target_url):
    ...
    try:
        ...
        date = re.search(r"(\d{4}\d{2}\d{2})", target_url)
        datetime_dir = re.match('(?P<year>\d{4})(?P<month>\d{2})(?P<day>\d{2})',
                                date[0])
    tt_tmp = '%s-%s-%s' % (
    ...
    _title = html_page.find('title').text.replace('/n','').strip()
