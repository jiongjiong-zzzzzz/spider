...
def standard_work_list():
    ...
    res = requests.get('http://www.fupin.gov.cn/helpnews/newslist-1.html', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='text-news-list').findAll('a')
    for one_info in infos:
        ...
        if content_dir:
            ...
            _url_tmp ='http://www.fupin.gov.cn/helpnews/'+ _url_tmp
...
def standard_work_article(target_url):
    ...
    reg_content = res.content.decode('utf8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='news-content').findAll('p')
    if  not infos:
        return
    for one_info in infos:
