...
def standard_work_list():
    ...
    res = requests.get('http://whlyj.beijing.gov.cn/xwzx/qxxw/index.htm', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf-8','ignore')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.select('.articleList2 ul a')
    for one_info in infos:
        ...
        if content_dir:
            _datetime = 0
            _url_tmp = 'http://whlyj.beijing.gov.cn/xwzx/qxxw/'+content_dir['href']
...
def standard_work_article(target_url):
    ...
    if not infos:
        return_data.append(html_page.find(class_='article').text)
