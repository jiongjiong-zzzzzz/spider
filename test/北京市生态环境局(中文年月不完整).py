...
def standard_work_list():
    ...
    res = requests.get('http://sthjj.beijing.gov.cn/bjhrb/index/hbdt/index.html', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf-8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.select('.h_wz_m ul li a')
    for one_info in infos:
        _one_info = str(one_info)
        content_dir = one_info
        if content_dir:
            _datetime = 0
            _url_tmp = 'http://sthjj.beijing.gov.cn'+content_dir['href'].strip()
...
def standard_work_article(target_url):
    ...
    reg_content = res.content.decode('utf-8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='h_dl_c').findAll('p')
    for one_info in infos:
