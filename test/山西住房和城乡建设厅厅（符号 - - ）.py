...
def standard_work_list():
    ...
    res = requests.get('http://zjt.shanxi.gov.cn/Main/main.action', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf-8','ignore')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.select('.js_dt_T li a')
    for one_info in infos:
        ...
        if content_dir:
            _datetime = 0
            _url_tmp = content_dir['href'].replace('../','http://zjt.shanxi.gov.cn/')
...
def standard_work_article(target_url):
    ...
    reg_content = res.content.decode('utf-8','ignore')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='Article_c').findAll('p')
    if not infos:
        return
    for one_info in infos:
    ...
    _title = html_page.select('.Article_a')[0].text
