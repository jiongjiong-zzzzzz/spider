...
def standard_work_list():
    ...
    res = requests.get('http://www.phirda.com/News.aspx?type=4', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='news_top').findAll('a')
    for one_info in infos:
        _one_info = str(one_info)
        if 'img ' in _one_info:
            content_dir = re.match('<a href="(?P<url>.+?)" target="_blank">(?P<title>.+?)<img src="/images/news.png"/></a>', _one_info)
        else:
            content_dir = re.match('<a href="(?P<url>.+?)" target="_blank">(?P<title>.+?)</a>', _one_info)
        if content_dir:
            _datetime = 0
            _url_tmp = 'http://www.phirda.com'+content_dir['url'].strip()
...
def standard_work_article(target_url):
    ...
    reg_content = res.content.decode('utf8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='xq_con').findAll('p')
    for one_info in infos:
