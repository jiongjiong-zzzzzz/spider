...
def standard_work_list():
    ...
    res = requests.get('http://gxs.guiyang.gov.cn/c7566/index.html', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf-8','ignore')
    infos = re.findall(r'''<A title='(?P<title>.+?)' href='(?P<url>.+?)' target='_blank' style="font-size:14px">.+?</A>''', reg_content)
