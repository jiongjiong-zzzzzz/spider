...
def standard_work_list():
    ...
    res = requests.get('http://sft.shanxi.gov.cn/SXSFWEB/sxsfweb_zwyw/sxsfweb_stdt', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf-8','ignore')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.select('a.sx_list_link')
...
def standard_work_article(target_url):
    ...
    reg_content = res.content.decode('utf-8','ignore')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='sx_artic').findAll('p')
    if not infos:
        return
    for one_info in infos:
