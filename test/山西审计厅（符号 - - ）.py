...
def standard_work_list():
    ...
    res = requests.get('http://sjt.shanxi.gov.cn/xwpd_279/sjxwlb/', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf-8','ignore')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.select('.lists_right li a')
    for one_info in infos:
        ...
        if content_dir:
            _datetime = 0
            _url_tmp = content_dir['href'].replace('./','http://sjt.shanxi.gov.cn/xwpd_279/sjxwlb/')
...
def standard_work_article(target_url):
    ...
    _title = html_page.find(class_='news_document').text
