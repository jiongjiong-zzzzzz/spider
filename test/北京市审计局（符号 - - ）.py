...
def standard_work_list():
    ...
    res = requests.get('http://sjj.beijing.gov.cn/zwxw/zhdt//', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('gbk','ignore')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.select('.area_list ul a')
    for one_info in infos:
        ...
        if content_dir:
            _datetime = 0
            _url_tmp = 'http://sjj.beijing.gov.cn'+content_dir['href']
...
def standard_work_article(target_url):
    ...
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    return_data.append(html_page.find(class_='content').text)
