...
def standard_work_list():
    ...
    url='http://www.guandian.cn/api.php?op=getindex2016_content&modelid=1&type=query-1&a=0&jsoncallback=jQuery191007480533407739265_1546853584259&_=1546853584263'
    ...
    for one_info in data:
        _datetime = 0
        _url_tmp='http://www.guandian.cn'+one_info['mainURL']
        title=one_info['titleText']
...
def standard_work_article(target_url):
    ...
    infos = html_page.find(class_='con_l_inner').findAll('p')
    ...
    time=html_page.find(class_='con_l_info_l').find('p').text.strip()
...
def main():
   ...
   print(list)
   print(standard_work_article(list[0]['url']))
