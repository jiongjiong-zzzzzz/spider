...
def standard_work_list():
    ...
    res = requests.get('http://world.chinadaily.com.cn/5bd55927a3101a87ca8ff614', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.select('.busBox3 div div h3 a')
    for one_info in infos:
        ...
        if content_dir:
            _datetime = 0
            _url_tmp = 'http:'+content_dir['href'].strip()
