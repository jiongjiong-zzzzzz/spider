...
def standard_work_list():
    ...
    res = requests.get('http://czt.hebei.gov.cn/xwdt/gzdt/', headers=headers)
    #res.raise_for_status()
    reg_content = res.content.decode('utf-8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.select('.list_tylbt_zhx a')
    for one_info in infos:
        ...
        if content_dir:
            ...
            if '../../' in _url_tmp:
                _url_tmp = _url_tmp.replace('../../', 'http://czt.hebei.gov.cn/')
                continue
            _url_tmp=_url_tmp.replace('./','http://czt.hebei.gov.cn/xwdt/gzdt/')
