...
def standard_work_list():
    ...
    res = requests.get('http://tech.163.com/telecom', headers=headers)
    ...
    for one_info in infos:
        ...
        content_dir = re.match('<h3 class="bigsize "><a href="(?P<url>.+?)">(?P<title>.+?)</a></h3>', _one_info)
        if content_dir:
            _datetime = 0
...
def standard_work_article(target_url):
    ...
    reg_content = res.content.decode('gbk')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='post_text').findAll('p')
    for one_info in infos:
