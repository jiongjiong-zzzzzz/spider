...
def standard_work_list():
    ...
    res = requests.get('http://www.acftu.org/template/10041/column.jsp?cid=222', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('GBK')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.select('ul.ul_list li a')
    for one_info in infos:
        _one_info = str(one_info)
        content_dir = one_info
        if content_dir:
            _datetime = 0
            _url_tmp ='http://www.acftu.org/template/10041/'+ content_dir['href']
