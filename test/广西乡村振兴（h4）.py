...
def standard_work_list():
    ...
    res = requests.get('http://www.gxxczx.com/fzdt/', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('gb2312','ignore')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.findAll('h4')
    for one_info in infos:
        _one_info = str(one_info)
        content_dir = re.match('<h4 class="media-heading"><a href="(?P<url>.+?)" target="_blank".+>(?P<title>.+?)</a></h4>', _one_info)
        if content_dir:
            _datetime = 0
            _url_tmp = content_dir['url'].strip()
            if 'www' not in _url_tmp:
                _url_tmp = 'http://www.gxxczx.com' + _url_tmp
...
def standard_work_article(target_url):
    ...
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    try:
        infos = html_page.find(id='article-text').findAll('p')
    except:
        infos = html_page.find(id='main-body').findAll('p')
    for one_info in infos:
    ...
    try:
        tim = html_page.find(class_='info-time-source').text.strip()
        ...
        datetime_dir = re.match('(?P<year>\d{4})-(?P<month>\d+?)-(?P<day>\d+?) (?P<hour>\d+?):(?P<minute>\d+)',
                                tim[0])
        tt_tmp = '%s-%s-%s %s:%s' % (
            datetime_dir['year'], datetime_dir['month'], datetime_dir['day'], datetime_dir['hour'],
            datetime_dir['minute'])
        _datetime = 0
        if datetime_dir:
