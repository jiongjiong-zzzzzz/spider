...
def standard_work_list():
    ...
    res = requests.get('http://www.gov.cn/zhengce/zuixin.htm', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf-8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='list list_1 list_2').findAll('a')
    for one_info in infos:
        ...
        if content_dir:
            ...
            if 'www.gov.cn' not in _url_tmp:
                _url_tmp = 'http://www.gov.cn'+_url_tmp
...
def standard_work_article(target_url):
    ...
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    try:
        infos = html_page.find(class_='b12c').findAll('p')
    except:
        infos = html_page.find(class_='pages_content').findAll('p')
    for one_info in infos:
