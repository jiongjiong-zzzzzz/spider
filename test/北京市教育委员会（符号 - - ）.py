...
def standard_work_list():
    ...
    res = requests.get('http://jw.beijing.gov.cn/jyzx/jyxw/', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('UTF-8','ignore')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.select('.announce_list ul a')
    for one_info in infos:
        _one_info = str(one_info)

        content_dir = one_info
        if 'guowuyuan' in _one_info:
            continue
        if content_dir:
            _datetime = 0
            _url_tmp = content_dir['href'].replace('./','http://jw.beijing.gov.cn/jyzx/jyxw/')
