...
def standard_work_list():
    ...
    res = requests.get('http://www.women.org.cn/col/col17/index.html', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf8')
    infos = re.findall('<div class="bt_title"><a href="(?P<url>.+?)" target="_blank">(?P<title>.+?)</a></div>', reg_content)
