...
def standard_work_list():
    ...
    res = requests.get('http://yjgl.guizhou.gov.cn/xwdt/yjyw/index.shtml', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf-8','ignore')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.select('.neirongbox li a')
    for one_info in infos:
        ...
        if content_dir:
            ...
            if 'http' not in _one_info:
                _url_tmp = 'http://yjgl.guizhou.gov.cn'+content_dir['href']
...
def standard_work_article(target_url):
    ...
    reg_content = res.content.decode('utf-8','ignore')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='detail_main_content').text
    ...
    _title = html_page.find(class_='detail_main_title').text.replace('\n','')
