...
def standard_work_list():
    ...
    res = requests.get('http://finance.eastmoney.com/news/cgspl.html', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('UTF-8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.findAll('p',{'class':'title'})
    for one_info in infos:
        _one_info = str(one_info).replace('\r\n','').replace('\n','')
        if 'title' not in _one_info:
            continue
        content_dir = re.match('<p class="title"><a href="(?P<url>.+?)" target="_blank">(?P<title>.+?)</a></p>', _one_info)
        if content_dir:
            _datetime = 0
...
def standard_work_article(target_url):
    ...
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(id='ContentBody').findAll('p')
    for one_info in infos:
