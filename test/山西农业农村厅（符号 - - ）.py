...
def standard_work_list():
    ...
    res = requests.get('http://nynct.shanxi.gov.cn/sndt/nyyw/', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf-8','ignore')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.select('.glcon li a')
    for one_info in infos:
        ...
        if content_dir:
            ...
            if '../../' in _url_tmp:
                _url_tmp = _url_tmp.replace('../../', 'http://nynct.shanxi.gov.cn/')
            _url_tmp = _url_tmp.replace('./', 'http://nynct.shanxi.gov.cn/sndt/nyyw/')
