...
def standard_work_list():
    ...
    res = requests.get('http://finance.people.com.cn/GB/153179/153180/index.html', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('GB2312')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='ej_left').findAll('a')
    for one_info in infos:
        _one_info = str(one_info)
        content_dir = re.match('<a href="(?P<url>.+?)" target="_blank">(?P<title>.+?)</a>', _one_info)
        if content_dir:
            _datetime = 0
            _url_tmp = 'http://finance.people.com.cn'+content_dir['url'].strip()
...
def standard_work_article(target_url):
    ...
    if not return_data:
        return_data.append(html_page.find(id='rwb_zw').text)
