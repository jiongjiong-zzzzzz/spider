...
def standard_work_list():
    ...
    res = requests.get('http://mw.jl.gov.cn/xwzx/wjdt/', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf-8','ignore')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.select('div.topc > p > a')
    for one_info in infos:
        ...
        if content_dir:
            ...
            _url_tmp = _url_tmp.replace('./','http://mw.jl.gov.cn/xwzx/wjdt/')
...
def standard_work_article(target_url):
    ...
    if datetime_dir:
        ...
        _title = html_page.find(class_='xq_bt').text
