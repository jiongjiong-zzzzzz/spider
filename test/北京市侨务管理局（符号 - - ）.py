...
def standard_work_list():
    ...
    res = requests.get('http://qb.beijing.gov.cn/web/static/catalogs/catalog_2c94968944a9a3100144a9d852760001/2c94968944a9a3100144a9d852760001.html', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('UTF-8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.select('.list03 li a')
    for one_info in infos:
        ...
        if content_dir:
            ...
            if 'http' not in _url_tmp:
                _url_tmp = 'http://qb.beijing.gov.cn/'+_url_tmp
...
def standard_work_article(target_url):
    ...
    try:
        ...
        infos = html_page.find(class_='atctxt').findAll('p')
    for one_info in infos:
