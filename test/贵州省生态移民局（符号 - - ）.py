...
def standard_work_list():
    ...
    res = requests.get('http://stymj.guizhou.gov.cn/ArticleList.aspx?ClassCode=SNYW', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf-8','ignore')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.select('.list li a')
    for one_info in infos:
        ...
        if content_dir:
            ...
            if 'http' not in _one_info:
                _url_tmp ='http://stymj.guizhou.gov.cn/'+content_dir['href']
...
def standard_work_article(target_url):
    ...
    reg_content = res.content.decode('utf-8','ignore')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='dan-nr').text
    ...
    _title = html_page.find(class_='dan-title').text.replace('\n','').replace('\t','').replace('\r','')
