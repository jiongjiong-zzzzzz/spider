...
def standard_work_list():
    ...
    res = requests.get('http://www.spp.gov.cn/spp/qwfb/index.shtml', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='ul_list active').findAll('a')
    for one_info in infos:
        ...
        if content_dir:
            ...
            if 'www' not in _url_tmp:
                _url_tmp = 'http://www.spp.gov.cn'+_url_tmp
...
def standard_work_article(target_url):
    ...
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(id='fontzoom').findAll('p')
    for one_info in infos:
