...
def standard_work_list():
    ...
    res = requests.get('http://swt.shanxi.gov.cn/', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf-8','ignore')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.select('.slideTxtBox.fl.h_342 .Lst01 li a')
    for one_info in infos:
        _one_info = str(one_info)
        if 'www.gov' in _one_info:
            continue
        content_dir = one_info
        if content_dir:
            _datetime = 0
            _url_tmp = content_dir['href'].replace('../','http://swt.shanxi.gov.cn/')
...
def standard_work_article(target_url):
    ...
    try:
        _title = html_page.select('.M_tit03')[0].text
    except:
