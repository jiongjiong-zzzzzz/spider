...
def standard_work_list():
    ...
    res = requests.get('http://www.cnii.com.cn/node_33989.htm', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf-8','ignore')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='list2').findAll('a')
    for one_info in infos:
        _one_info = str(one_info)
        content_dir = one_info
        if content_dir:
            _datetime = 0
            _url_tmp = 'http://www.cnii.com.cn/'+content_dir['href'].strip()
...
def standard_work_article(target_url):
    ...
    infos = html_page.find(class_='conzw').findAll('p')
    ...
    if not return_data:
        return_data.append(html_page.find(class_='conzw').text)
