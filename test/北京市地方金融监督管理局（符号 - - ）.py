...
def standard_work_list():
    ...
    res = requests.get('http://jrj.beijing.gov.cn/gzdt/c12-list-1.html', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('UTF-8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.select('.erjiUL ul li a')
    for one_info in infos:
        ...
        if content_dir:
            _datetime = 0
            _url_tmp = 'http://jrj.beijing.gov.cn/gzdt/'+content_dir['href']
...
def standard_work_article(target_url):
    ...
    reg_content = res.content.decode('UTF-8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='articleWord').findAll('p')
    for one_info in infos:
