...
def standard_work_list():
    ...
    res = requests.get('http://www.ciita.org.cn/1/68.shtml', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('gb2312')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='zpd_list').findAll('a')
    for one_info in infos:
        _one_info = str(one_info)
        if 'mag_tt' in _one_info or 'default' in _one_info or 'cysdjd_nr_rt' in _one_info or 'copybd_a' in _one_info or '...' in _one_info:
            continue
        content_dir = one_info
        if content_dir:
            _datetime = 0
            _url_tmp = content_dir['href'].strip()
            _url_tmp ='http://www.ciita.org.cn' + _url_tmp
...
def standard_work_article(target_url):
    ...
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='article_content').findAll('p')
    for one_info in infos:
    ...
    if not return_data:
        return_data.append(html_page.find(class_='article_content').text)
    ...
    _title = html_page.find(class_='article_title').text
