...
def standard_work_list():
    ...
    res = requests.get('http://www.chinaql.org/GB/419628/419641/index.html', headers=headers)
    ...
    for one_info in infos:
        ...
        if content_dir:
            _datetime = 0
            _url_tmp = 'http://www.chinaql.org'+content_dir['url'].strip()
            if 'search.chinaql' in _url_tmp:
                continue
...
def standard_work_article(target_url):
    ...
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='detail-text').findAll('p')
    for one_info in infos:
    ...
    if not return_data:
        return_data.append(html_page.find(class_='list_left').text)
