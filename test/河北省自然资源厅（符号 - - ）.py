...
def standard_work_list():
    ...
    res = requests.get('http://zrzy.hebei.gov.cn/heb/gtxw/xw/styw/', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf-8','ignore')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.select('.p_nrnei li a')
    for one_info in infos:
        ...
        if content_dir:
            _datetime = 0
            _url_tmp = 'http://zrzy.hebei.gov.cn'+content_dir['href']
