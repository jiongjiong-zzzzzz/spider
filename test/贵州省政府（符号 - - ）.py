...
def standard_work_list():
    ...
    res = requests.get('http://www.guizhou.gov.cn/xwdt/gzyw/', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf-8','ignore')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.select('.right-list-box li a')
...
def standard_work_article(target_url):
    ...
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    try:
        infos = html_page.find(class_='zw-con').findAll('p')
    except:
        return
    if not infos:
        return
