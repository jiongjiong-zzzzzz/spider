...
def standard_work_list():
    ...
    res = requests.get('http://www.chinawuliu.com.cn/xsyj/class_46.shtml', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('UTF-8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='box1_L4').findAll('a')
    for one_info in infos:
        ...
        if content_dir:
            _datetime = 0
            _url_tmp = 'http://www.chinawuliu.com.cn'+content_dir['href']
...
def standard_work_article(target_url):
    ...
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='main1_L8').findAll('p')
    if not infos:
        return
    for one_info in infos:
