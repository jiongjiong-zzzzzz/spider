...
def standard_work_list():
    ...
    res = requests.get('http://www.cae.cn/cae/html/main/col1/column_1_1.html', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('UTF-8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='right_md_list').findAll('a')
    for one_info in infos:
        ...
        if content_dir:
            _datetime = 0
            _url_tmp = 'http://www.cae.cn'+content_dir['href']
