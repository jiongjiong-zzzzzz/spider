...
def standard_work_list():
    ...
    res = requests.get('http://www.mca.gov.cn/article/xw/mzyw/', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='article').findAll('a')
    for one_info in infos:
        _one_info = str(one_info)
        content_dir = one_info
        if content_dir:
            _datetime = 0
            _url_tmp ='http://www.mca.gov.cn'+ content_dir['href'].strip()
...
def standard_work_article(target_url):
    ...
    _title = html_page.find('h3').text.replace('\r\n\t','')
