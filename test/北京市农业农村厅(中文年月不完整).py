...
def standard_work_list():
    ...
    res = requests.get('http://nyj.beijing.gov.cn/nyj/232120/233036/index.html', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf-8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.select('ul.list a')
    for one_info in infos:
        _one_info = str(one_info)
        content_dir = one_info
        if content_dir:
            _datetime = 0
            _url_tmp = 'http://nyj.beijing.gov.cn'+content_dir['href'].strip()
...
def standard_work_article(target_url):
    ...
    reg_content = res.content.decode('utf-8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='easysite-news-text').findAll('p')
    for one_info in infos:
