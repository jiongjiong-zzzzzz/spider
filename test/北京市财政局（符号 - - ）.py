...
def standard_work_list():
    ...
    res = requests.get('http://czj.beijing.gov.cn/zwxx/czyw/index.htm', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('gbk','ignore')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.select('.ul-back li a')
    for one_info in infos:
        ...
        if content_dir:
            _datetime = 0
            _url_tmp = content_dir['href'].replace('./','http://czj.beijing.gov.cn/zwxx/czyw/')
