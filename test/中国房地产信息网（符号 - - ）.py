...
def standard_work_list():
    ...
    res = requests.get('http://www.realestate.cei.gov.cn/file/index.aspx?type1=%BD%F1%C8%D5%D2%AA%CE%C5&op=tt&b=0&k=1', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf-8','ignore')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.select('a.lawbra')
