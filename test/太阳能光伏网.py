...
def standard_work_list():
    ...
    res = requests.get('https://solar.ofweek.com/CATList-2600-8100-solar.html', headers=headers)
    ...
    for one_info in infos:
        ...
        if '<img alt' in str(one_info):
            content_dir = re.match('<h3><a href="(?P<url>.+?)" target="_blank"><img alt="" src=".+" title=""/>(?P<title>.+?)</a></h3>',
                                   _one_info)
        else:
...
def standard_work_article(target_url):
    ...
    reg_content = res.content.decode('GBK')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(id='articleC').findAll('p')
    for one_info in infos:
    ...
    datetime_dir = re.match('(?P<year>\d{4})-(?P<month>\d+?)-(?P<day>\d+?) (?P<hour>\d+?):(?P<minute>\d+?)',
                            html_page.find(class_='sdate').text.strip())
    tt_tmp = '%s-%s-%s %s:%s' % (
