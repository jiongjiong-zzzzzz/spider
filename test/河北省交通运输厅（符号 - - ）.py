...
def standard_work_list():
    ...
    res = requests.get('http://jtt.hebei.gov.cn/jtyst/jtzx/styw/', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf-8','ignore')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.select('.zc_p_lbRight li a')
    for one_info in infos:
        ...
        if content_dir:
            _datetime = 0
            _url_tmp = 'http://jtt.hebei.gov.cn'+content_dir['href']
...
def standard_work_article(target_url):
    ...
    reg_content = res.content.decode('utf-8','ignore')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(id='BodyLabel').text
    ...
    _title = html_page.select('.p_nei h1')[0].text
