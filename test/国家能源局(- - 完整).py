...
def standard_work_list():
    ...
    res = requests.get('http://www.nea.gov.cn/xwzx/nyyw.htm', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf-8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='box01').findAll('a')
    for one_info in infos:
        _one_info = str(one_info)
        if 'target' not in _one_info or 'leaders' in _one_info or 'fms' in _one_info:
            continue
...
def standard_work_article(target_url):
    ...
    try:
        ...
        try:
            infos = html_page.find(class_='article-content').findAll('p')
        except:
