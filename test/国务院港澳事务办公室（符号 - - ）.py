...
def standard_work_list():
    ...
    res = requests.get('http://www.hmo.gov.cn/xwzx/gayw/', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('UTF-8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='itemList').findAll('a')
    for one_info in infos:
        ...
        if content_dir:
            _datetime = 0
            _url_tmp = content_dir['href'].replace('./','http://www.hmo.gov.cn/xwzx/gayw/').replace('amp;','')
...
def standard_work_article(target_url):
    ...
    reg_content = res.content.decode('UTF-8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='detailCon').findAll('p')
    for one_info in infos:
