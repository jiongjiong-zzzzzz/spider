...
def standard_work_list():
    ...
    res = requests.get('http://www.moa.gov.cn/xw/zwdt/', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='pub-media1-txt-list fullwidth').findAll('a')
    for one_info in infos:
        _one_info = str(one_info)
        if 'index' in _one_info or 'vod.mwr.gov' in _one_info or 'javascript' in _one_info:
            continue
        ...
        if content_dir:
            ...
            _url_tmp =_url_tmp.replace('./','http://www.moa.gov.cn/xw/zwdt/')
