...
def standard_work_list():
    ...
    res = requests.get('http://news.pharmnet.com.cn/news/gjdt/index0.html', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('gb2312','ignore')
    infos = re.findall('<a href="(?P<url>.+?)" target=_blank class="u">(?P<title>.+?)</a>', reg_content)
...
def standard_work_article(target_url):
    ...
    infos = html_page.find(class_='ct02').text
