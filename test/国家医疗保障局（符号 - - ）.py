...
def standard_work_list():
    ...
    res = requests.get('http://www.nhsa.gov.cn/col/col14/index.html', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('UTF-8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = re.findall('<a href="(?P<url>.+?)" title="(?P<titl>.+?)" target="_blank">(?P<title>.+?)</a>', reg_content)
    ...
    for one_info in infos:
        ...
        if content_dir:
            _datetime = 0
            _url_tmp = 'http://www.nhsa.gov.cn'+content_dir[0]
