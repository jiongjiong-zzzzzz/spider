...
def standard_work_list():
    ...
    res = requests.get('http://www.ec.com.cn/list/dssz/1/cateinfo.html', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='news-items').findAll('a')
    for one_info in infos:
        _one_info = str(one_info)
        if 'img' in _one_info or 'cysdjd_nr_lt' in _one_info or 'cysdjd_nr_rt' in _one_info or 'copybd_a' in _one_info or '...' in _one_info:
            continue
        content_dir = one_info
        if content_dir:
            _datetime = 0
            _url_tmp ='http://www.ec.com.cn'+ content_dir['href'].strip()
...
def standard_work_article(target_url):
    ...
    datetime_dir = re.match('(?P<year>\d{4})-(?P<month>\d+?)-(?P<day>\d+?) (?P<hour>\d+?):(?P<minute>\d+?):\d+',
                            html_page.find(id='article_pubtime').text.strip())
    tt_tmp = '%s-%s-%s %s:%s' % (
    ...
    _title = html_page.find(id='article_title').text
