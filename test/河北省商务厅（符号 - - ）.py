...
def standard_work_list():
    ...
    res = requests.get('http://swt.hebei.gov.cn/newsContentform/infoPublicList?channelId=620&y=1', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf-8','ignore')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.select('.normalTbl td a')
    for one_info in infos:
        ...
        if content_dir:
            ...
            if 'http' not in _url_tmp:
                _url_tmp = 'http://swt.hebei.gov.cn'+_url_tmp
...
def standard_work_article(target_url):
    ...
    reg_content = res.content.decode('utf-8','ignore')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='artile-con').text
    ...
    _title = html_page.select('.artile-con h3')[0].text
