...
def standard_work_list():
    ...
    res = requests.get('http://www.miit.gov.cn/n1146290/index.html', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='clist_con').findAll('a')
    for one_info in infos:
        ...
        if content_dir:
            ...
            if 'wenming' not in _url_tmp:
                _url_tmp = content_dir['href'].replace('../','http://www.miit.gov.cn/')
...
def standard_work_article(target_url):
    ...
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    try:
        infos = html_page.find(class_='ccontent center').findAll('p')
    except:
        try:infos = html_page.find(id='tex').findAll('p')
        except:return
    for one_info in infos:
