...
def standard_work_list():
    ...
    res = requests.get('http://mil.gmw.cn/node_8986.htm', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf-8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.select('ul.channel-newsGroup li a')
    for one_info in infos:
        ...
        if content_dir:
            ...
            if 'gmw' not in _url_tmp:
                _url_tmp = 'http://mil.gmw.cn/'+_url_tmp
...
def standard_work_article(target_url):
    ...
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    try:
        infos = html_page.find(class_='u-mainText').findAll('p')
    except:
        return
    for one_info in infos:
