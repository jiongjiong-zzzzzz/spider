...
def standard_work_list():
    ...
    res = requests.get('http://news.sciencenet.cn/fieldlist.aspx?id=7', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(id='DataGrid1').findAll('a')
    for one_info in infos:
        ...
        if content_dir:
            ...
            if 'http' not in _url_tmp:
                _url_tmp = 'http://news.sciencenet.cn'+_url_tmp
...
def standard_work_article(target_url):
    ...
    reg_content = res.content.decode('utf8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    need_content = html_page.find(id='content1').text
    ...
    _titles = html_page.findAll('td',{'class':'style1'})
