...
def standard_work_list():
    ...
    res = requests.get('http://whly.hebei.gov.cn/Home/NewsCenter', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf-8','ignore')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.select('.tdn ul li a')
    for one_info in infos:
        ...
        if content_dir:
            ...
            if 'http' not in _url_tmp:
                _url_tmp = _url_tmp.replace('../','http://whly.hebei.gov.cn/')
...
def standard_work_article(target_url):
    ...
    reg_content = res.content.decode('utf-8','ignore')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='text').text.replace('\xa0','').replace('\n','')
    ...
    _title = html_page.select('.content h1')[0].text
