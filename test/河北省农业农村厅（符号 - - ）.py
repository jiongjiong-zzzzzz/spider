...
def standard_work_list():
    ...
    res = requests.get('http://nync.hebei.gov.cn/#', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf-8','ignore')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.select('.zwdt_cont ul li a')
    for one_info in infos:
        ...
        if content_dir:
            ...
            if 'http' not in _url_tmp:
                _url_tmp = 'http://nync.hebei.gov.cn'+_url_tmp
...
def standard_work_article(target_url):
    ...
    reg_content = res.content.decode('utf-8','ignore')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='content').text
    ...
    _title = html_page.select('.mtitle')[0].text
