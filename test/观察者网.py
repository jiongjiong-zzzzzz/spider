...
def standard_work_list():
    ...
    res = requests.get('https://www.guancha.cn/BaTie/list_1.shtml', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='column-list fix').findAll('a')
    quanwen = urllib.parse.unquote_plus('%E5%85%A8%E6%96%87')
    for one_info in infos:
        _one_info = str(one_info)
        if 'img' in _one_info or 'list' in _one_info or quanwen in _one_info or 'copybd_a' in _one_info or '...' in _one_info:
            continue
        content_dir = re.match('<a href="(?P<url>.+?)" target="_blank">(?P<title>.+?)</a>', _one_info)
        if content_dir:
            _datetime = 0
            _url_tmp = 'https://www.guancha.cn'+content_dir['url'].strip()
...
def standard_work_article(target_url):
    ...
    reg_content = res.content.decode('utf8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='content all-txt').findAll('p')
    for one_info in infos:
