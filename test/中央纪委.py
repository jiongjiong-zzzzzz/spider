...
def standard_work_list():
    ...
    res = requests.get('http://www.ccdi.gov.cn/xsxc/', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='list_news_dl fixed').findAll('a')
    for one_info in infos:
        ...
        if content_dir:
            _datetime = 0
            _url_tmp = content_dir['href'].replace('./','http://www.ccdi.gov.cn/xsxc/').strip()
