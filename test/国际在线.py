...
def standard_work_list():
    ...
    res = requests.get('http://mil.cri.cn/', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='list-a').findAll('a')
    for one_info in infos:
        _one_info = str(one_info)
        if 'img' in _one_info or 'list' in _one_info or 'cysdjd_nr_rt' in _one_info or 'copybd_a' in _one_info or '...' in _one_info:
            continue
        content_dir = re.match('<a href="(?P<url>.+?)" target="_blank">(?P<title>.+?)</a>', _one_info)
        if content_dir:
            _datetime = 0
            _url_tmp = content_dir['url'].replace('//','http://').strip()
            if 'news.cri.cn' not in _url_tmp:
                _url_tmp = 'http://mil.cri.cn'+content_dir['url'].strip()
...
def standard_work_article(target_url):
    ...
    reg_content = res.content.decode('utf8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(id='abody').findAll('p')
    for one_info in infos:
    ...
    datetime_dir = re.match('(?P<year>\d{4})-(?P<month>\d+?)-(?P<day>\d+?) (?P<hour>\d+?):(?P<minute>\d+?):\d+',
                            html_page.find(id='acreatedtime').text.strip())
    tt_tmp = '%s-%s-%s %s:%s' % (
