...
def standard_work_list():
    ...
    res = requests.get('http://minzheng.hebei.gov.cn/mzzx/mzyw/', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('gb2312','ignore')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = re.findall(r'<a href="(?P<url>.+?)" target="_blank" title="(?P<title>.+?)">.+?</a>',reg_content)
    for one_info in infos:
        ...
        if content_dir:
            _datetime = 0
            _url_tmp = content_dir[0].replace('./','http://minzheng.hebei.gov.cn/mzzx/mzyw/')
...
def standard_work_article(target_url):
    ...
    reg_content = res.content.decode('gb2312','ignore')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='Custom_UnionStyle').text.replace('\u3000','').replace('\xa0','')
    if not infos:
