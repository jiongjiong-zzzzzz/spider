...
def standard_work_list():
    ...
    res = requests.get('http://www.qstheory.cn/', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf-8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='col-md-4 newsBlock clearfix yw').findAll('a')
    info = html_page.find(class_='row ').findAll('a')
...
def standard_work_article(target_url):
    ...
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    try:
        infos = html_page.find(class_='highlight').findAll('p')
    except:
        try:
            infos = html_page.find(class_='Custom_UnionStyle').findAll('p')
        except:
            infos = html_page.find(class_='').findAll('p')
