...
def standard_work_list():
    ...
    res = requests.get('http://www.biotech.org.cn/topic/109', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf-8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(id='zuob').findAll('a')
    for one_info in infos:
        ...
        if content_dir:
            ...
            _url_tmp ='http://www.biotech.org.cn/' +_url_tmp
...
def standard_work_article(target_url):
    ...
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(id='nr').findAll('p')
    for one_info in infos:
    ...
    _title = html_page.find(id='caption').text
