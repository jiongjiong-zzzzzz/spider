...
def standard_work_list():
    ...
    res = requests.get('https://borneobulletin.com.bn/?s=the+Belt+and+Road', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf-8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.select('.td-ss-main-content h3.entry-title,td-module-title a')
    for one_info in infos:
        ...
        if content_dir:
            ...
            if 'index' in _url_tmp or 'web/adv.jsp' in _url_tmp:
                continue
            elif './' in _url_tmp:
                _url_tmp = _url_tmp.replace('./','http://www.stats.gov.cn/tjsj/zxfb/')
            else:
                _url_tmp = 'http://www.stats.gov.cn'+content_dir['href']
