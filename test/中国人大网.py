...
def standard_work_list():
    ...
    res = requests.get('http://www.npc.gov.cn/npc/xinwen/node_10134.htm', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('UTF-8',"ignore")
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='fl s_lw').findAll('a')
    for one_info in infos:
        _one_info = str(one_info)
        content_dir = re.match('<a href="(?P<url>.+?)" target="_blank">(?P<title>.+?)</a>', _one_info)
        if content_dir:
            _datetime = 0
            _url_tmp = 'http://www.npc.gov.cn/npc/xinwen/'+content_dir['url'].strip()
...
def standard_work_article(target_url):
    ...
    if not return_data:
        return_data.append(html_page.find(id='Zoom').text)
