...
def standard_work_list():
    ...
    res = requests.get('http://s.huanqiu.com/s?q=%E9%87%91%E6%AD%A3%E6%81%A9', headers=headers)
    ...
    for one_info in infos:
        ...
        if 'href' not in _one_info:
            continue
        content_dir_ = re.search('<a href=".*?" target="_blank" title=".*?">.*?</a>', _one_info)
        content = content_dir_[0]
        content_dir = re.match('<a href="(?P<url>.+?)" target="_blank".+>(?P<title>.+?)</a>',content )
        if content_dir:
            _datetime = 0
...
def standard_work_article(target_url):
    ...
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    try:
        infos = html_page.find(class_='la_con').findAll('p')
    except:
        return
    for one_info in infos:
