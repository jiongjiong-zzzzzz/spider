...
def standard_work_list():
    ...
    res = requests.get('http://www.tobacco.gov.cn/html/11.html', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('gb2312')
    print(reg_content)
    infos = re.findall('<a href="(?P<url>.+?)" onclick="count(88976932)" target="_blank">(?P<title>\s.+?\s)</a>', reg_content,re.S)
