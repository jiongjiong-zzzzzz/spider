...
def standard_work_list():
    ...
    res = requests.get('http://www.customs.gov.cn/customs/xwfb34/302262/302263/index.html', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf-8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='conList_ul').findAll('a')
    for one_info in infos:
        ...
        if content_dir:
            ...
            _url_tmp='http://www.customs.gov.cn'+_url_tmp
...
def standard_work_article(target_url):
    ...
    reg_content = res.content.decode('utf-8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='easysite-news-content').findAll('p')
    for one_info in infos:
