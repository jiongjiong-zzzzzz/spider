...
def standard_work_list():
    ...
    res = requests.get('http://news.cssn.cn/zx/bwyc/', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('UTF-8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='f-main-left').findAll('a')
    for one_info in infos:
        _one_info = str(one_info)
        content_dir = re.match('<a href="(?P<url>.+?)">(?P<title>.+?)</a>', _one_info)
        if content_dir:
            _datetime = 0
            _url_tmp = content_dir['url'].strip().replace('amp;','')
            if 'http' not in _url_tmp:
                _url_tmp = _url_tmp.replace('./','http://news.cssn.cn/zx/bwyc/')
            title = content_dir['title'].strip()
            if 'H5'in title:
                continue
