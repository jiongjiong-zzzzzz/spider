...
def standard_work_list():
    ...
    res = requests.get('http://www.audit.gov.cn/n4/n19/index.html', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf-8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.select('td.grey14_24 a')
    for one_info in infos:
        ...
        if content_dir:
            ...
            _url_tmp=_url_tmp.replace('../../','http://www.audit.gov.cn/')
...
def standard_work_article(target_url):
    ...
    reg_content = res.content.decode('utf-8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(id='ziti').findAll('p')
    for one_info in infos:
