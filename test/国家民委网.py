...
def standard_work_list():
    ...
    res = requests.get('http://www.seac.gov.cn/seac/xwzx/mwyw/index.shtml', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('UTF-8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.select('.w1 ul li a')
    for one_info in infos:
        ...
        if content_dir:
            _datetime = 0
            _url_tmp ='http://www.seac.gov.cn'+ content_dir['href']
...
def standard_work_article(target_url):
    ...
    reg_content = res.content.decode('UTF-8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='p3').findAll('p')
    for one_info in infos:
