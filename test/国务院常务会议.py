...
def standard_work_list():
    ...
    res = requests.get('http://www.ce.cn/ztpd/xwzt/guonei/gwyh/index.shtml', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('gb2312')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find('table',{'class':'con'}).findAll('td')
    for one_info in infos:
        _one_info = str(one_info)
        if 'href' not in _one_info:
            continue
        content_dir = re.match('<td bgcolor="#FFFFFF">(?P<title>.+?)<a.+href="(?P<url>.+?)" target="_blank">.+</a></td>', _one_info)
        if content_dir:
            _datetime = 0
