...
def standard_work_list():
    ...
    res = requests.get('http://www.mohrss.gov.cn/SYrlzyhshbzb/dongtaixinwen/buneiyaowen/', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='serviceMainListConType').findAll('a')
    for one_info in infos:
        _one_info = str(one_info)
        content_dir = one_info
        if content_dir:
            _datetime = 0
            _url_tmp = content_dir['href'].strip().replace('./','http://www.mohrss.gov.cn/SYrlzyhshbzb/dongtaixinwen/buneiyaowen/')
...
def standard_work_article(target_url):
    ...
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    try:
        infos = html_page.find(class_='TRS_PreAppend').findAll('p')
    except:return
    for one_info in infos:
