...
def standard_work_list():
    ...
    res = requests.get('http://www.cnmn.com.cn/ShowNewsList.aspx?id=43', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('UTF-8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='tab-pane active con-middle ').findAll('a')
    for one_info in infos:
        _one_info = str(one_info)
        if 'title' not in _one_info:
            continue
        content_dir = one_info
        if content_dir:
            _datetime = 0
            _url_tmp = 'http://www.cnmn.com.cn'+content_dir['href'].strip()
...
def standard_work_article(target_url):
    ...
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(id='txtcont').findAll('p')
    for one_info in infos:
    ...
    tim = html_page.find(class_='time').text.strip()
    nian = urllib.parse.unquote_plus('%E5%B9%B4')
    yue = urllib.parse.unquote_plus('%E6%9C%88')
    ri = urllib.parse.unquote_plus('%E6%97%A5')
    date = re.search(r"(\d{4}%s\d{1,2}%s\d{1,2}%s\s\d{1,2}:\d{1,2})"%(nian,yue,ri) , tim)
    datetime_dir = re.match('(?P<year>\d{4})%s(?P<month>\d+?)%s(?P<day>\d+?)%s (?P<hour>\d+?):(?P<minute>\d+)'%(nian,yue,ri),tim)
    tt_tmp = '%s-%s-%s %s:%s' % (
    ...
    _title = html_page.find(class_='h4title').text
