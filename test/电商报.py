...
def standard_work_list():
    ...
    res = requests.get('http://www.dsb.cn/tag/lingshou?page=1', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='new-list-cons').findAll('a')
    for one_info in infos:
        _one_info = str(one_info)
        if 'img' in _one_info or 'target="_blank"' not in _one_info:
            continue
...
def standard_work_article(target_url):
    ...
    reg_content = res.content.decode('utf8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='new-content-con').findAll('p')
    for one_info in infos:
