...
def standard_work_list():
    ...
    res = requests.get('http://www.cflac.org.cn/tt_more/', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf-8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='art_list').findAll('a')
    for one_info in infos:
        ...
        if content_dir:
            ...
            if 'xinhuanet' in _url_tmp or 'wywzt' in _url_tmp:
                continue
            _url_tmp = _url_tmp.replace('./','http://www.cflac.org.cn/tt_more/')
