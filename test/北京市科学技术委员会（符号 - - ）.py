...
def standard_work_list():
    ...
    res = requests.get('http://kw.beijing.gov.cn/col/col44/index.html', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('UTF-8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = re.findall(r'''<a style="font-size:15px;" href='(?P<url>.+?)' title='.+?' target='_blank' >(?P<title>.+?)</a>''',reg_content)
    for one_info in infos:
        _one_info = str(one_info)
        content_dir = one_info
        if content_dir:
            _datetime = 0
            _url_tmp = 'http://kw.beijing.gov.cn'+content_dir[0]
