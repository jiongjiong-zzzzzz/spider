...
def standard_work_list():
    ...
    res = requests.get('http://tzcj.guiyang.gov.cn/c8557/index.html', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf-8','ignore')
    infos = re.findall(r'''<a href='(.+?)' title='(.+?)' style='font-size:10.5pt; line-height:27px' target='_blank'>.+?</a>''',reg_content)
...
def standard_work_article(target_url):
    ...
    _title = re.findall(r'''<td height="35" align='center'  style="font-weight:bold; font-size:18px; border-bottom:solid 1px #cccccc;">(.+?)<br></td>''', reg_content,re.S)
    return _datetime, '%s<replace title>%s' % (_title[0], '\n'.join(return_data))
