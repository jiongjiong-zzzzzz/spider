...
def standard_work_list():
    ...
    res = requests.get('http://www.gxny.gov.cn/xwdt/ywkb/', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='ulthree').findAll('a')
    for one_info in infos:
        ...
        if content_dir:
            ...
            _url_tmp = _url_tmp.replace('./','http://www.gxny.gov.cn/xwdt/ywkb/')
...
def standard_work_article(target_url):
    ...
    infos = html_page.find(id='mcontent').findAll('p')
