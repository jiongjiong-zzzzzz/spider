...
def standard_work_list():
    ...
    res = requests.get('http://www.hebei.gov.cn/hebei/14462058/14471802/14471717/index.html', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf-8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.select('.szf_ejlb_List li a')
    for one_info in infos:
        ...
        if content_dir:
            _datetime = 0
            _url_tmp = 'http://www.hebei.gov.cn'+content_dir['href'].strip()
