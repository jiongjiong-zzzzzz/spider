...
def standard_work_list():
    ...
    res = requests.get('http://column.cankaoxiaoxi.com/g/china/', headers=headers)
    ...
    for one_info in infos:
        ...
        content_dir = re.match('<h2><a href="(?P<url>.+?)" target="_blank">(?P<title>.+?)</a></h2>', _one_info)
        if content_dir:
            _datetime = 0
...
def standard_work_article(target_url):
    ...
    reg_content = res.content.decode('utf8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='articleText').findAll('p')
    for one_info in infos:
