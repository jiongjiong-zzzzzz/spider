...
def standard_work_list():
    ...
    res = requests.get('http://gat.shanxi.gov.cn/#/policenew', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf-8','ignore')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.select('.el-col,el-col-18 a')
    for one_info in infos:
        ...
        if content_dir:
            _datetime = 0
            _url_tmp = 'http://gat.shanxi.gov.cn'+content_dir['href']
...
def standard_work_article(target_url):
    ...
    reg_content = res.content.decode('utf-8','ignore')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='el-main.bs-content').findAll('p')
    infos = html_page.find(class_='el-main.bs-content').text
