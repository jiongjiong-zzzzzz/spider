...
def standard_work_list():
    ...
    res = requests.get('http://news.12371.cn/dzybmbdj/zzb/', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('UTF-8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(id='SUBD1512093243376896').findAll('a')
    for one_info in infos:
        _one_info = str(one_info)
        if 'special'  in _one_info or 'dzybmbdj'  in _one_info:
            continue
...
def standard_work_article(target_url):
    ...
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='word').findAll('p')
    for one_info in infos:
