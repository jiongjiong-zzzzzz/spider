...
def standard_work_list():
    ...
    res = requests.get('http://www.mof.gov.cn/zhengwuxinxi/caizhengxinwen/', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('gb2312')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.select('td.ZITI a')
    for one_info in infos:
        ...
        if content_dir:
            ...
            _url_tmp = _url_tmp.replace('../../','http://www.mof.gov.cn/')

            _url_tmp = _url_tmp.replace('../', 'http://www.mof.gov.cn/zhengwuxinxi/')
            _url_tmp = _url_tmp.replace('./', 'http://www.mof.gov.cn/zhengwuxinxi/caizhengxinwen/')
