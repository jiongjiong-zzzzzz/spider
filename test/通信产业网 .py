...
def standard_work_list():
    ...
    res = requests.get('http://www.ccidcom.com/yaowen/index.html', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('UTF-8','ignore')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='artlisting').findAll('a')
    for one_info in infos:
        _one_info = str(one_info)
        content_dir = re.match('<a href="(?P<url>.+?)" target="_blank"><font color="">(?P<title>.+?)</font></a>', _one_info)
        if content_dir:
            _datetime = 0
            _url_tmp = 'http://www.ccidcom.com'+content_dir['url'].strip()
            print({'url': _url_tmp, 'title': content_dir['title']})
            return_data.append({'url': _url_tmp, 'title': content_dir['title']  , 'datetime': _datetime})
    return return_data
def standard_work_article(target_url):
    ...
    datetime_dir = re.match('(?P<year>\d{4})-(?P<month>\d+?)-(?P<day>\d+?) (?P<hour>\d+?):(?P<minute>\d+?)', date[0].strip())
    tt_tmp = '%s-%s-%s %s:%s' % (
    ...
    _title = html_page.find(class_='heading').text
