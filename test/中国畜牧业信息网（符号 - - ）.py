...
def standard_work_list():
    ...
    res = requests.get('http://www.caaa.cn/market/index.php?class=242', headers=headers)
    ...
    for one_info in infos:
        ...
        if content_dir:
            _datetime = 0
            _url_tmp = 'http://www.caaa.cn'+content_dir['href']
...
def standard_work_article(target_url):
    ...
    reg_content = res.content.decode('UTF-8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='font_14').findAll('p')
    for one_info in infos:
