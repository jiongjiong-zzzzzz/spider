...
def standard_work_list():
    ...
    res = requests.get('http://www.21jingji.com/channel/opinion/', headers=headers)
    ...
    jujiao = urllib.parse.unquote_plus('%E8%81%9A%E7%84%A6')
    for one_info in infos:
        content_dir=one_info
        if jujiao not in content_dir['title']:
            _datetime = 0
...
def standard_work_article(target_url):
    ...
    reg_content = res.content.decode('utf8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='txtContent').findAll('p')
    for one_info in infos:
    ...
    datetime_dir = re.match('(?P<year>\d{4})-(?P<month>\d+?)-(?P<day>\d+?) (?P<hour>\d+?):(?P<minute>\d+?)',
                            html_page.find(class_='newsDate').text.strip())
