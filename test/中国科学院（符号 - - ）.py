...
def standard_work_list():
    ...
    res = requests.get('http://www.cas.cn/yw/', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('UTF-8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='ztlb_ld_mainR_box01_list').findAll('a')
    for one_info in infos:
        ...
        if content_dir:
            _datetime = 0
            _url_tmp = content_dir['href'].replace('./','http://www.cas.cn/yw/').replace('amp;','')
