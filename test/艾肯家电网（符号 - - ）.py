...
def standard_work_list():
    ...
    res = requests.get('http://www.abi.com.cn/news/news-more.asp?lb=12&chksm=1559720165', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf-8','ignore')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.select('h5 a.fl.bluefont')
    for one_info in infos:
        _one_info = str(one_info)
        content_dir = one_info
        if 'ebook' in _one_info:
            continue
        if content_dir:
            _datetime = 0
...
def standard_work_article(target_url):
    ...
    try:
        infos = html_page.find(class_='content-font').findAll('p')
        for one_info in infos:
        ...
        return_data.append(html_page.find(class_='content').text.strip())
