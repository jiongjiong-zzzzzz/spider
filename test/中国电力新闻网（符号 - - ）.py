...
def standard_work_list():
    ...
    res = requests.get('http://www.cpnn.com.cn/dw/default.htm', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('gb2312')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='cpnn-content-left-list').findAll('a')
    for one_info in infos:
        _one_info = str(one_info)
        if 'target' not in _one_info or 'leaders' in _one_info or 'live1' in _one_info or 'weixin' in _one_info:
            continue
        content_dir = one_info
        if content_dir:
            _datetime = 0
            _url_tmp = content_dir['href'].replace('./','http://www.cpnn.com.cn/dw/')
...
def standard_work_article(target_url):
    ...
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='cpnn-con-zhenwen').findAll('p')
    for one_info in infos:
    ...
    riqi = urllib.parse.unquote_plus('%e6%97%a5%e6%9c%9f%ef%bc%9a')
    date = re.search(r"%s(\d{2}.\d{1,2}.\d{1,2})"%(riqi), reg_content)
    date_ = re.search(r"(\d{2}.\d{1,2}.\d{1,2})", date[0])
    datetime_dir = re.match('(?P<year>\d{2}).(?P<month>\d+?).(?P<day>\d+)',
                            date_[0])
    tt_tmp = '%s-%s-%s' % (
