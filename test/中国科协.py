...
def standard_work_list():
    ...
    res = requests.get('http://www.cast.org.cn/col/col79/index.html', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf8')
    infos = re.findall('<p><a href="(?P<url>.+?)" target="_blank">(?P<title>.+?)</a></p>', reg_content)

    for one_info in infos:

        _datetime = 0
        _url_tmp = 'http://www.cast.org.cn'+one_info[0]
        print({'url': _url_tmp, 'title': one_info[1].strip()})
        return_data.append({'url': _url_tmp, 'title': one_info[1].strip(), 'datetime': _datetime})
    return return_data
...
def standard_work_article(target_url):
    ...
    _title = html_page.find(class_='topp').text.replace('\r\n','')
