...
def standard_work_list():
    ...
    res = requests.get('http://www.chinasafety.gov.cn/xw/byw/', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf-8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='center_display_right').findAll('a')
    for one_info in infos:
        ...
        if content_dir:
            ...
            _url_tmp=_url_tmp.replace('./','http://www.chinasafety.gov.cn/xw/byw/')
