...
def standard_work_list():
    ...
    res = requests.get('http://www.sastind.gov.cn/n112/n117/index.html', headers=headers)
    ...
    for one_info in infos:
        ...
        if 'n112' not in content_dir[0] or 'content' not in content_dir[0]:
            continue
        if content_dir:
            _datetime = 0
            _url_tmp = content_dir[0].replace('../../','http://www.sastind.gov.cn/')
...
def standard_work_article(target_url):
    ...
    reg_content = res.content.decode('UTF-8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(id='con_con').findAll('p')
    for one_info in infos:
