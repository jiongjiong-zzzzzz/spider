...
def standard_work_list():
    ...
    url='http://app.cnfol.com/qualityarticles/qualityarticles.php?CatId=135&starttime=1547420106&endtime=1547434506&num=20&page=1&record=2&jsoncallback=callback&_=1547434506504'
    res = requests.get(url,headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf-8')
    html_json = re.findall(r'callback[(]{"flag":"1001","list":(.+?),"record":3}[)]',reg_content,re.S)
...
def standard_work_article(target_url):
    ...
    need_content = html_page.find(class_='Article').text
    return_data.append(need_content.strip())
    tim = re.search(r"(\d{4}-\d{1,2}-\d{1,2}\s\d{1,2}:\d{1,2}:\d{1,2})", reg_content)
    datetime_dir = re.match('(?P<year>\d{4})-(?P<month>\d+?)-(?P<day>\d+?) (?P<hour>\d+?):(?P<minute>\d+?):\d+',tim[0] )
    tt_tmp = '%s-%s-%s %s:%s' % (
    ...
    _title = html_page.find(class_='artTitle').text
