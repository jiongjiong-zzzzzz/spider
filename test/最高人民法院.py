...
Cookie = '_gscu_125736681=52272961ndduke59; _gscbrs_125736681=1; Hm_lvt_9e03c161142422698f5b0d82bf699727=1552272967,1552871596; wzws_cid=9767098539b6a2a0a07f08e89b6fd3d0db423c7ac98a308ae635d2c02e675fa3e6128daffb987e93642ac7d1b7028f29; COURTID=s35ls1pa3kiosndg2lc45bjck7; _gscs_125736681=52871592qid4fj18|pv:9; Hm_lpvt_9e03c161142422698f5b0d82bf699727=1552872547'
Referer = 'http://www.court.gov.cn/zixun.html'
...
def standard_work_list():
    ...
    headers = {'User-Agent': UserAgent,
               'Cookie':Cookie,
               'Referer':Referer}
    res = requests.get('http://www.court.gov.cn/zixun-gengduo-23.html', headers=headers)
    ...
    for one_info in infos:
        ...
        if content_dir:
            ...
            _url_tmp ='http://www.court.gov.cn'+_url_tmp
...
def standard_work_article(target_url):
    ...
    reg_content = res.content.decode('utf8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='txt big').findAll('p')
    if  not infos:
        return
    for one_info in infos:
    ...
    _title = html_page.find('title').text.replace('/n','').replace('/t','').strip()
