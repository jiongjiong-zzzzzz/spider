...
Cookie = 'JSESSIONID=971488AAFE2DD32BAC02DF1BEF8B8597.tomcat2; Hm_lvt_b8386baf89eebcc2cea019180d947981=1552011918; Hm_lpvt_b8386baf89eebcc2cea019180d947981=1552012304'
...
def standard_work_list():
    ...
    headers = {'User-Agent': UserAgent,
               'Cookie':Cookie}
    res = requests.get('http://www.cww.net.cn/subjects/nav/rollList/12372', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf-8','ignore')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='pindao mt0').findAll('a')
    for one_info in infos:
        _one_info = str(one_info)
        content_dir = one_info
        if 'img' in content_dir:
            continue
        if content_dir:
            _datetime = 0
...
def standard_work_article(target_url):
    ...
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    try:
        infos = html_page.find(id='divContentDiv').findAll('p')
    except:
        return
    for one_info in infos:
