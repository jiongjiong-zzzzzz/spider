...
def standard_work_list():
    ...
    res = requests.get('http://kjt.hebei.gov.cn/eportal/ui?pageId=91573', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf-8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.select('.buwei_div ul li a')
    for one_info in infos:
        ...
        if content_dir:
            _datetime = 0
            _url_tmp ='http://kjt.hebei.gov.cn'+ content_dir['href'].strip()
...
def standard_work_article(target_url):
    ...
    reg_content = res.content.decode('utf-8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='xiangqnr').findAll('p')
    for one_info in infos:
