...
def standard_work_list():
    ...
    res = requests.get('http://www.sasac.gov.cn/n2588025/index.html', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('UTF-8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.select('a.clearfix')
    for one_info in infos:
        ...
        if content_dir:
            ...
            _url_tmp = _url_tmp.replace('../','http://www.sasac.gov.cn/')
...
def standard_work_article(target_url):
    ...
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    try:
        infos = html_page.find(class_='zsy_comain').findAll('p')
    except:
        return
    for one_info in infos:
