...
def standard_work_list():
    ...
    res = requests.get('http://wsjkw.hebei.gov.cn/index.do?cid=45&templet=gzdt_list', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('utf-8','ignore')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.select('.sy_new_list a')
    for one_info in infos:
        ...
        if content_dir:
            ...
            if 'http' not in _url_tmp:
                _url_tmp = 'http://wsjkw.hebei.gov.cn/'+_url_tmp
...
def standard_work_article(target_url):
    ...
    reg_content = res.content.decode('utf-8','ignore')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='con_wz').text.replace('\u3000','').replace('\n','')
    ...
    _title = html_page.find(class_='con_bt').text
