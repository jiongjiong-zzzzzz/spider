...
def standard_work_list():
    ...
    res = requests.get('http://www.chinaisa.org.cn/gxportal/DispatchAction.do?efFormEname=ECTM40&key=CmlcYw1mWTgKa1NkUjUHZgZiBGcIbFJlBzQDMAJhATZWRQ1CDhUENgYXUhVUQwxs', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('UTF-8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.select('a.subpage_inflist_text')
    for one_info in infos:
        _one_info = str(one_info)
        content_dir = one_info
        if content_dir:
            _datetime = 0
            _url_tmp = 'http://www.chinaisa.org.cn'+content_dir['href']
...
def standard_work_article(target_url):
    ...
    reg_content = res.content.decode('UTF-8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='Section1').text

    return_data.append(infos.strip())
    date = re.search(r"(\d{4}\d{2}\d{2})", reg_content)
    datetime_dir = re.match('(?P<year>\d{4})(?P<month>\d{2})(?P<day>\d+)',
                            date[0])
    tt_tmp = '%s-%s-%s' % (
