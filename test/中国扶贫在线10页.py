#encoding=utf8
import requests,re,bs4,sys,hashlib,uuid,json,base64,rsa,platform,datetime,os,urllib
import time
...
def standard_work_list():
    ...
    for page in range(2, 11):
        res = requests.get('http://f.china.com.cn/node_7237055_%s.htm'%(page), headers=headers)
...
def standard_work_article(target_url):
    ...
    try:
        tim = html_page.find('h2').text
        datetime_dir = re.match('.+:(?P<year>\d{4})-(?P<month>\d+?)-(?P<day>\d+?) (?P<hour>\d+?):(?P<minute>\d+?):.+',
                                tim)
    except AttributeError:
        tim = html_page.find(id='pubtime_baidu').text.strip()
        tim = re.search(r"(\d{4}-\d{1,2}-\d{1,2}\s\d{1,2}:\d{1,2}:\d{1,2})",time)
        tim = tim.group(0)
        datetime_dir = re.match('(?P<year>\d{4})-(?P<month>\d+?)-(?P<day>\d+?) (?P<hour>\d+?):(?P<minute>\d+?):\d+',tim)
    tt_tmp = '%s-%s-%s %s:%s' % (
