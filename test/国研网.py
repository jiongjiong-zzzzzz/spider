...
def standard_work_list():
    ...
    res = requests.get('http://www.drcnet.com.cn/www/int/Leaf.aspx?uid=050201&version=integrated&chnid=1020&leafid=3025', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('gb2312','ignore')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='aa14').findAll('a')
...
def standard_work_article(target_url):
    ...
    infos = html_page.find(id='docContent').findAll('p')
    ...
    if not return_data:
        return_data.append(html_page.find(class_='main-body').text)
    tim = re.search(r"(\d{4}-\d{1,2}-\d{1,2})", reg_content)
    datetime_dir = re.match('(?P<year>\d{4})-(?P<month>\d+?)-(?P<day>\d+)',tim[0])
    tt_tmp = '%s-%s-%s' % (
