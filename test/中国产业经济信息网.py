...
def standard_work_list():
    ...
    res = requests.get('http://www.cinic.org.cn/index.php?m=content&c=index&a=lists&catid=17', headers=headers)
    ...
    for one_info in infos:
        ...
        if content_dir:
            ...
            _url_tmp = 'http://www.cinic.org.cn'+_url_tmp
...
def standard_work_article(target_url):
    ...
    reg_content = res.content.decode('utf8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.find(class_='dc-ccm1').findAll('p')
    for one_info in infos:
        content_dir = re.search('<p[\\s\\S]+/p>', str(one_info))
        if content_dir:
            need_content = one_info.text
            if '010-65363056' in need_content:
                break
        else:
    ...
    if not return_data:
        infos = html_page.find(class_='dc-ccm1').findAll('div')
        for one_info in infos:
            content_dir = re.search('<div[\\s\\S]+/div>', str(one_info))
            if content_dir:
                need_content = one_info.text
                if '010-65363056' in need_content:
                    break
