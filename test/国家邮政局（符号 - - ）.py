...
def standard_work_list():
    ...
    res = requests.get('http://www.spb.gov.cn/xw/dsjxx_1/', headers=headers)
    res.raise_for_status()
    reg_content = res.content.decode('UTF-8')
    html_page = bs4.BeautifulSoup(reg_content, 'lxml')
    infos = html_page.findAll('a',{'class':'postcnx'})
    for one_info in infos:
        ...
        if content_dir:
            _datetime = 0
            _url_tmp = content_dir['href'].replace('./','http://www.spb.gov.cn/xw/dsjxx_1/')
...
def standard_work_article(target_url):
    ...
    _title = html_page.find(class_='title1').text
